export const SEO_BASE_URL = 'https://www.yogayterapiasarunachala.es';
...
export function resolveBreadcrumbs(
    ...
    if (meditationMatch) {
        ...
        return [
            ...base,
            {
                name: meditationMatch[1].replace(/-/g, ' '),
                path,
            },
        ];
    }
...
export function mergeStructuredDataWithBreadcrumbs(
    structuredData: object | object[] | undefined,
    breadcrumbSchema: object | null
): object | object[] | undefined {
    if (!breadcrumbSchema) return structuredData;
    if (!structuredData) return breadcrumbSchema;
    if (Array.isArray(structuredData)) return [...structuredData, breadcrumbSchema];
    return [structuredData, breadcrumbSchema];
}

            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {reviews.map((review) => (
                        <a
                            key={review.id}
                            href={review.author_url || googleUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="group block bg-white/50 backdrop-blur-sm p-6 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 border border-bark/5 hover:-translate-y-1"
                        >
                            ...
                            <p className="text-bark/80 mb-4 italic line-clamp-4 group-hover:text-bark transition-colors">"{review.text}"</p>
                            <div className="flex justify-between items-end border-t border-bark/10 pt-4">
                                <span className="font-bold text-forest group-hover:text-matcha transition-colors">{review.author}</span>

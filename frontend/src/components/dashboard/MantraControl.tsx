...
export default function MantraControl() {
    ...
    const handleSetDailyClick = async (id: number) => {
        if (dailyMantra?.id === id) return;
        setIsLoading(true);
        try {
            const token = sessionStorage.getItem('access_token');
            const response = await fetch(`${API_BASE_URL}/api/mantras/${id}/set-daily`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (response.ok) {
                const data = await response.json();
                setDailyMantra(data);
                setMessage({ type: 'success', text: 'Mantra fijado como diario' });
            } else {
                setMessage({ type: 'error', text: 'Error al fijar el mantra diario' });
            }
        } catch (error) {
            ...
            <div className="mt-8">
                ...
                <div className="flex flex-wrap gap-3">
                    {allMantras.map((mantra) => (
                        <div
                            key={mantra.id}
                            onClick={() => handleSetDailyClick(mantra.id)}
                            className={`px-4 py-2 rounded-full text-sm flex items-center gap-2 group transition-all cursor-pointer ${dailyMantra?.id === mantra.id

...
export default function PromotionManager() {
    ...
    const [activeTab, setActiveTab] = useState<'tarifas' | 'promociones'>('tarifas');
    ...
    const [isTarifa, setIsTarifa] = useState(false);
    ...
    const handleEdit = (item: Promotion) => {
        ...
        setIsTarifa(item.is_tarifa ?? false);
        setPreviewUrl(getImageUrl(item.image_url));
        setActiveTab(item.is_tarifa ? 'tarifas' : 'promociones');
        setShowModal(true);
    };
    ...
    const handleCloseModal = () => {
        ...
        setEndDate('');
        setIsActive(true);
        // keep isTarifa as the active tab context if we were just closing
        setIsTarifa(activeTab === 'tarifas');
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Tarifas y Promociones</h1>
                    <p className="text-sm text-gray-500">Gestiona las tarifas de actividades y las promociones y ofertas de la web.</p>
                </div>
                <button
                    onClick={() => {
                        setIsTarifa(activeTab === 'tarifas');
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg hover:bg-indigo-500 transition-all shrink-0"
                >
                    <PlusIcon className="h-5 w-5" />
                    {activeTab === 'tarifas' ? 'Nueva Tarifa' : 'Nueva Promoción'}
                </button>
            </div>
            ...
            <div className="flex space-x-2 border-b border-gray-200">
                <button
                    ...
                    className={`py-3 px-6 text-sm font-bold border-b-2 transition-colors ${activeTab === 'tarifas' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                >
                    Tarifas
                </button>
                <button
                    onClick={() => setActiveTab('promociones')}
                    className={`py-3 px-6 text-sm font-bold border-b-2 transition-colors ${activeTab === 'promociones' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                >
                    Promociones
                </button>
            </div>
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {items.filter(item => activeTab === 'tarifas' ? item.is_tarifa : !item.is_tarifa).map((item) => (
                        <div key={item.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                            ...
                            <div className="p-5 flex-1">
                                ...
                                <div className="mt-4 flex items-center gap-2">
                                    {item.price && <span className="px-2 py-1 bg-forest/10 text-forest rounded text-xs font-bold tracking-wider">{item.price}</span>}
                ...
                <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center z-[200] p-4">
                    <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center">
                            <h3 className="text-xl font-bold text-gray-900">{editingItem ? (isTarifa ? 'Editar Tarifa' : 'Editar Promoción') : (isTarifa ? 'Nueva Tarifa' : 'Nueva Promoción')}</h3>
                            <button onClick={() => { if (!isCropModalOpen) handleCloseModal(); }} className="p-2 hover:bg-gray-100 rounded-full text-gray-400 transition-colors"> &times; </button>
                        </div>
                        <form onSubmit={handleSubmit} className="p-8 space-y-5 overflow-y-auto">
                            {/* Hidden toggle, handled by creating from specific tab but keeping logic for safety */}
                            {false && (
                                <div className="flex items-center justify-between p-4 bg-forest/5 rounded-2xl border border-forest/10">
                                    <div>
                                        <p className="text-sm font-bold text-gray-900">Registrar como Tarifa Regular</p>
                                    </div>
                                    <button type="button" onClick={() => setIsTarifa(!isTarifa)} className={`w-12 h-6 rounded-full transition-all relative ${isTarifa ? 'bg-forest' : 'bg-gray-300'}`}>
                                        <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${isTarifa ? 'left-7' : 'left-1'}`} />
                                    </button>
                                </div>
                            )}

                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">{isTarifa ? 'Nombre de la Tarifa / Actividad *' : 'Título *'}</label>
                                <input type="text" required value={title} onChange={e => setTitle(e.target.value)} className="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 p-3 border" placeholder={isTarifa ? "Ej: Yoga Mensual" : "Ej: Oferta de Invierno"} />
                            </div>
                            ...
                            {isTarifa ? (
                                <>
                                    <div>
                                        ...
                                        <input type="text" required={isTarifa} value={price} onChange={e => setPrice(e.target.value)} className="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 p-3 border" placeholder="Ej: 50€ / mes" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-gray-700 mb-1">Días y Horarios</label>
                                        <textarea rows={2} value={description} onChange={e => setDescription(e.target.value)} className="w-full rounded-xl border-gray-200 focus:border-indigo-500 focus:ring-indigo-500 p-3 border" placeholder="Ej: 2 días a la semana (Lunes y Miércoles)" />
                                    </div>
                                </>
                            ) : (
                                <>
                            ...
                            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl mt-4">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">{isActive ? (isTarifa ? 'Visible permanentemente en la web' : 'Visible en la web si está en fecha') : 'Borrador oculto'}</p>
                                </div>

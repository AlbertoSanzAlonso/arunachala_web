...
const MeditationsPage: React.FC = () => {
    ...
    const filteredMeditations = useMemo(() => {
        return meditations.filter((meditation) => {
            ...
            const displayExcerpt = (translation?.excerpt || translation?.body || meditation.excerpt || meditation.body || '').toLowerCase();
        ...
        const fetchMeditations = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    // Filter out virtual tracks (negative IDs are local-only background music)
                    setMeditations((data || []).filter((m: Meditation) => m.id > 0));
                } else {
                    setMeditations([]);
                }
            } catch (error) {
            ...
            <main className="flex-grow pt-4 md:pt-16 pb-16 relative">
                <div className="max-w-7xl mx-auto px-6 relative w-full">
                    ...
                    <motion.h1
                        ...
                        className="text-4xl md:text-6xl font-headers text-forest text-center mb-6 uppercase tracking-wider pt-0"
                    >
                        {t('meditations.title')}
                    </motion.h1>
                    <motion.p
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                                <AnimatePresence>
                                    {currentItems.map((meditation, index) => {
                                        ...
                                        const displayExcerpt = translation?.excerpt || translation?.body || meditation.excerpt || meditation.body;

...
class Content(Base):
    ...
    type = Column(String, index=True, nullable=False)  # ContentType
    category = Column(String, index=True, nullable=True)  # 'yoga', 'therapy', 'general'
    status = Column(String, index=True, default=ContentStatus.DRAFT)
...
class ClassSchedule(Base):
    ...
    note = Column(String, nullable=True)  # Optional short note shown in schedule card
    translations = Column(JSONB, nullable=True)  
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
...
class Activity(Base):
    ...
    description = Column(Text, nullable=True)
    translations = Column(JSONB, nullable=True)
    type = Column(String, index=True, nullable=False) # 'curso', 'taller', 'evento', 'retiro'
...
class Suggestion(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    activity_id = Column(Integer, ForeignKey("activities.id"), index=True, nullable=True) # Link to the specific poll activity
    ...
    status = Column(String, index=True, default="pending") # pending, reviewed, implemented
...
class Promotion(Base):
    ...
    end_date = Column(DateTime(timezone=True), nullable=True)
    is_active = Column(Boolean, index=True, default=True)
    is_tarifa = Column(Boolean, default=False)  # Tarifa vs Promocion
    translations = Column(JSONB, nullable=True)
    # RAG sync fields
    vector_id = Column(String, nullable=True)
    vectorized_at = Column(DateTime(timezone=True), nullable=True)

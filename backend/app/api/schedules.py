...
from app.models.models import ClassSchedule, User, Activity, DashboardActivity, Subscription
import json
...
async def notify_subscribers_schedule_change(schedule_id: int, db_session_factory, notification_type: str = "new", deleted_data: dict = None):
    db = db_session_factory()
    try:
        schedule_data = None

        if deleted_data:
            schedule_data = deleted_data
        else:
            schedule = db.query(ClassSchedule).filter(ClassSchedule.id == schedule_id).first()
            if not schedule:
                return
            
            title = schedule.yoga_class.name if schedule.yoga_class else schedule.class_name or "Clase"
            translations = schedule.yoga_class.translations if schedule.yoga_class and getattr(schedule.yoga_class, 'translations', None) else None
        ...
        await email_service.send_schedule_notification(recipients, schedule_data, notification_type)
    except Exception as e:
        print(f"Error in notify_subscribers_schedule_change: {e}")
    finally:
        db.close()
...
def get_schedules(
    ...
    query = db.query(ClassSchedule).options(joinedload(ClassSchedule.yoga_class))
    if active_only:
...
async def create_schedule(
    ...
    new_schedule = ClassSchedule(
        ...
        translations=schedule_data.translations
    )
    ...
    # Auto-translate note on creation
    if new_schedule.note:
    ...
    if new_schedule.is_active:
        background_tasks.add_task(
            notify_subscribers_schedule_change,
            new_schedule.id,
            SessionLocal,
            "new"
        )
...
async def update_schedule(
    ...
    if schedule_data.note is not None:
        schedule.note = schedule_data.note
    if schedule_data.translations is not None:
        schedule.translations = schedule_data.translations
    ...
    # Re-translate note if it changed
    if schedule.note and schedule.note != original_note:
        background_tasks.add_task(
            auto_translate_background,
            SessionLocal,
            ClassSchedule,

...
def get_recent_activity(
    ...
    for item in persistent_activities:
        activities.append({
            ...
            'type': item.type,
            'action': item.action,
            'title': item.title if item.action != 'deleted' else f"{item.title} (Eliminado)",
    ...
    for content in contents:
        ...
        activities.append({
            ...
            'type': 'content',
            'action': 'created',
            'title': f"{type_label}: {content.title}",
    ...
    for item in gallery_items:
        activities.append({
            ...
            'type': 'gallery',
            'action': 'created',
            'title': f"Imagen de galería ({item.category or 'general'})",
    ...
    for yoga_class in yoga_classes:
        activities.append({
            ...
            'type': 'yoga_class',
            'action': 'created',
            'title': f"Clase de yoga: {yoga_class.name}",
    ...
    for schedule in schedules:
        ...
        activities.append({
            ...
            'type': 'schedule',
            'action': 'created',
            'title': f"Horario: {class_name} - {schedule.day_of_week} {schedule.start_time}",
    ...
    for activity in activity_items:
        activities.append({
            ...
            'type': 'activity',
            'action': 'created',
            'title': f"Actividad: {activity.title}",
        ...
        if activity.updated_at and activity.updated_at != activity.created_at:
            activities.append({
                ...
                'type': 'activity',
                'action': 'updated',
                'title': f"Actividad actualizada: {activity.title}",

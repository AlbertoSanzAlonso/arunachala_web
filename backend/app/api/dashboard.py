...
def get_recent_activity(
    ...
    def add_activity(entity_id, type_name, action, title, timestamp, author_name=None):
        if not timestamp:
            return
    ...
    # 2. Add Content
    # We load the author as well to get author_name
    from sqlalchemy.orm import joinedload
    contents = db.query(Content).options(joinedload(Content.author)).order_by(desc(func.coalesce(Content.updated_at, Content.created_at))).limit(limit).all()
    for content in contents:
        ...
        author_name = content.author.first_name if content.author else None
        
        if content.updated_at and content.updated_at != content.created_at:
            add_activity(content.id, 'content', 'updated', f"{type_label} actualizado: {content.title}", content.updated_at, author_name)
        add_activity(content.id, 'content', 'created', f"{prefix} {type_label}: {content.title}", content.created_at, author_name)
    ...
    # 5. Add Schedules
    schedules = db.query(ClassSchedule).options(joinedload(ClassSchedule.yoga_class)).order_by(desc(func.coalesce(ClassSchedule.updated_at, ClassSchedule.created_at))).limit(limit).all()
    for s in schedules:

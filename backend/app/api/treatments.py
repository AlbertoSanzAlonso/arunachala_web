...
async def update_massage(
    ...
    # Capture original fields before modification
    original_name = db_massage.name
    original_excerpt = db_massage.excerpt
    original_description = db_massage.description
    original_benefits = db_massage.benefits
    ...
    if db_massage.name != original_name: needs_translation = True
    if db_massage.excerpt != original_excerpt: needs_translation = True
    if db_massage.description != original_description: needs_translation = True
    if db_massage.benefits != original_benefits: needs_translation = True

    if needs_translation:
        # Keep empty strings to ensure stale translations are cleared
        fields = {
            "name": db_massage.name, 
            "excerpt": db_massage.excerpt, 
            "description": db_massage.description, 
            "benefits": db_massage.benefits,
            "price": db_massage.price
        }
        fields = {k: v for k, v in fields.items() if v is not None}
        print(f"🔄 QUEUEING TRANSLATION for Massage #{db_massage.id} (fields: {list(fields.keys())})")
        background_tasks.add_task(
            auto_translate_background, 
            SessionLocal, 
            MassageType, 
...
async def update_therapy(
    ...
    # Capture original fields before modification
    original_name = db_therapy.name
    original_excerpt = db_therapy.excerpt
    original_description = db_therapy.description
    original_benefits = db_therapy.benefits
    ...
    # Re-translate if fields changed and no new translations provided
    needs_translation = False
    if db_therapy.name != original_name: needs_translation = True
    if db_therapy.excerpt != original_excerpt: needs_translation = True
    if db_therapy.description != original_description: needs_translation = True
    if db_therapy.benefits != original_benefits: needs_translation = True

    if needs_translation:
        # Keep empty strings to ensure stale translations are cleared
        fields = {
            "name": db_therapy.name, 
            "excerpt": db_therapy.excerpt, 
            "description": db_therapy.description, 
            "benefits": db_therapy.benefits,
            "price": db_therapy.price
        }
        fields = {k: v for k, v in fields.items() if v is not None}
        print(f"🔄 QUEUEING TRANSLATION for Therapy #{db_therapy.id} (fields: {list(fields.keys())})")
        background_tasks.add_task(
            auto_translate_background, 
            SessionLocal, 
            TherapyType, 

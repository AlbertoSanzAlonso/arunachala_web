...
def create_or_update_mantra(
    ...
    background_tasks.add_task(auto_translate_background, SessionLocal, Mantra, mantra.id, fields_to_translate)
...
@router.post("/{mantra_id}/set-daily", response_model=MantraSchema)
def set_mantra_as_daily(
    mantra_id: int,
    current_user: User = Depends(get_current_admin_user),
    db: Session = Depends(get_db)
):
    mantra = db.query(Mantra).filter(Mantra.id == mantra_id).first()
    if not mantra:
        raise HTTPException(status_code=404, detail="Mantra no encontrado")

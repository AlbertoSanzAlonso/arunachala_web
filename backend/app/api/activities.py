...
async def notify_subscribers_activity_change(activity_id: int, db_session_factory, notification_type: str = "new", deleted_data: dict = None):
    ...
    try:
        ...
        is_sugerencia = activity_data.get("type") == "sugerencia"
        if is_sugerencia:
            await email_service.send_suggestion_notification(recipients, activity_data, activity_url, notification_type)
        else:
...
def get_activities(db: Session = Depends(get_db), active_only: bool = True):
    ...
    # Enrich suggestion activities with vote data and comments (Optimized: 1 query instead of N+1)
    sugerencia_ids = [a.id for a in activities if a.type == 'sugerencia']
    if sugerencia_ids:
        # Fetch all suggestions for these activities in one go
        all_suggestions = db.query(Suggestion).filter(Suggestion.activity_id.in_(sugerencia_ids)).order_by(Suggestion.created_at.desc()).all()
        
        # Group suggestions by activity_id
        suggestions_by_activity = {}
        for s in all_suggestions:
            if s.activity_id not in suggestions_by_activity:
                suggestions_by_activity[s.activity_id] = []
            suggestions_by_activity[s.activity_id].append(s)
            
        for activity in activities:
            if activity.type == 'sugerencia':
                activity_suggestions = suggestions_by_activity.get(activity.id, [])
                
                vote_results = {}
                custom_proposals = {}
                regular_comments = []
                
                for s in activity_suggestions:
                    # 1. Process standard votes (with support for multiple selections)
                    if s.activity_type and s.activity_type != 'custom':
                        options = [opt.strip() for opt in s.activity_type.split(',') if opt.strip()]
                        for opt in options:
                            vote_results[opt] = vote_results.get(opt, 0) + 1
                    
                    # 2. Process custom proposals
                    if s.activity_type == 'custom' and s.custom_suggestion:
                        ...
                        if s.created_at and (not custom_proposals[proposal_text]['date'] or s.created_at > custom_proposals[proposal_text]['date']):
                            custom_proposals[proposal_text]['date'] = s.created_at
                    
                    # 3. Process regular comments
                    else:
                        text = s.comments or ""
                        if text:
                ...
                activity.vote_results = vote_results
                activity.user_comments = []
                
                # Add grouped custom proposals to user_comments
                p_items = sorted(custom_proposals.values(), key=lambda x: x['votes'], reverse=True)
                for p_data in p_items:
                    activity.user_comments.append({
                        "text": p_data['text'],
                        "option": "custom",
                        "votes": p_data['votes'],
                        "date": p_data['date'].isoformat() if p_data['date'] else None
                    })
...
def get_activity(activity_id: int, db: Session = Depends(get_db)):
    ...
    if activity.type == 'sugerencia':
        # Calculate vote results with support for multiple selections
        suggestions_for_votes = db.query(Suggestion.activity_type)\
                  .filter(Suggestion.activity_id == activity.id).all()
        
        vote_results = {}
        for row in suggestions_for_votes:
            if row[0]:
                # Multiple selections are stored as comma-separated values
                options = [opt.strip() for opt in row[0].split(',') if opt.strip()]
                for opt in options:
                    vote_results[opt] = vote_results.get(opt, 0) + 1
...
async def create_activity(
    ...
    # Auto-translate if no translations provided or translations are empty
    is_translations_empty = not translations or translations == "{}" or translations == "null"
    if is_translations_empty and background_tasks:
        ...
        try:
            ...
            if isinstance(raw_ad, dict) and "options" in raw_ad:
                ...
                if "custom_field_label" in raw_ad and raw_ad["custom_field_label"]:
                    translation_fields["custom_field_label"] = raw_ad["custom_field_label"]
        except Exception as e:
            print(f"Error parsing raw activity_data for translation: {e}")

        # Final sanity check to avoid sending empty lists to AI
        translation_fields = {k: v for k, v in translation_fields.items() if v}
...
async def update_activity(
    ...
    original_data = activity.activity_data or {}
    original_options = []
    original_custom_field_label = ""
    if activity.activity_data and isinstance(activity.activity_data, dict):
        if "options" in activity.activity_data:
            original_options = [o.get('text', '') if isinstance(o, dict) else str(o) for o in (original_data.get("options") or []) if o]
    original_custom_field_label = original_data.get("custom_field_label", "")
    original_translations = dict(activity.translations or {})

    if title is not None: activity.title = title
    if description is not None: activity.description = description
    
    # Protect translations: only overwrite if provided and not effectively empty
    if translations is not None:
        try:
            parsed_trans = json.loads(translations)
            # If incoming is empty but we have existing, protect them
            if (not parsed_trans or len(parsed_trans) == 0) and activity.translations and len(activity.translations) > 0:
                print(f"🛡️ Protecting existing translations for Activity #{activity_id}")
            else:
                activity.translations = parsed_trans
        except:
            pass
    ...
    if (activity.title or "").strip() != (original_title or "").strip(): needs_translation = True
    if (activity.description or "").strip() != (original_description or "").strip(): needs_translation = True
    
    new_options = []
    if activity.activity_data and isinstance(activity.activity_data, dict) and "options" in activity.activity_data:
        new_options = [o.get('text', '') if isinstance(o, dict) else str(o) for o in activity.activity_data["options"] if o]
    
    # Normalize options for comparison
    def normalize_list(lst): return sorted([str(x).strip() for x in lst if x])
    if normalize_list(new_options) != normalize_list(original_options):
        needs_translation = True
        
    new_custom_field_label = ""
    if activity.activity_data and isinstance(activity.activity_data, dict):
        new_custom_field_label = activity.activity_data.get("custom_field_label", "")
    if (new_custom_field_label or "").strip() != (original_custom_field_label or "").strip():
        needs_translation = True

    # Re-translate if main fields changed OR translations are missing
    is_translations_missing = not activity.translations or len(activity.translations) == 0
    
    # Check if user manually changed translations in this request
    manual_trans_change = False
    if translations is not None:
        try:
            # Parse if string
            provided_trans = json.loads(translations) if isinstance(translations, str) else translations
            if provided_trans != original_translations:
                manual_trans_change = True
        except:
            manual_trans_change = True

    if needs_translation or (is_translations_missing and not manual_trans_change):
        # Use RAW activity_data from form to ensure we have the newest edits
        translation_fields = {"title": activity.title}
        if activity.description:
            translation_fields["description"] = activity.description
            
        if new_options:
            translation_fields["options"] = new_options
        
        if activity.activity_data and isinstance(activity.activity_data, dict) and activity.activity_data.get("custom_field_label"):
            translation_fields["custom_field_label"] = activity.activity_data["custom_field_label"]

        # Keep empty strings to ensure stale translations are cleared
        translation_fields = {k: v for k, v in translation_fields.items() if v is not None}
        
        if translation_fields:
            print(f"🔄 QUEUEING TRANSLATION for Activity #{activity.id} (fields: {list(translation_fields.keys())})")
            background_tasks.add_task(
                auto_translate_background, 
                SessionLocal, 
                Activity, 

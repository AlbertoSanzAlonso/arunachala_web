...
from app.models.models import AutomationTask, User, YogaClassDefinition, ClassSchedule
...
@router.post("/sync-translations")
async def sync_translations(
    background_tasks: BackgroundTasks,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Force re-translation for all yoga classes and schedules to ensure consistency"""
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Not authorized")
    
    # 1. Classes
    classes = db.query(YogaClassDefinition).all()
    count_classes = 0
    for yc in classes:
        # Check if it needs re-syncing (e.g. missing age_range in translations)
        # For simplicity, we'll just queue all that have any content
        fields = {
            "name": yc.name,
        ...
        if fields:
            background_tasks.add_task(auto_translate_background, SessionLocal, YogaClassDefinition, yc.id, fields)
            count_classes += 1
    ...
    for s in schedules:
        if s.note:
            background_tasks.add_task(auto_translate_background, SessionLocal, ClassSchedule, s.id, {"note": s.note})
            count_schedules += 1
            
    return {
        "message": "Sincronización de traducciones iniciada en segundo plano.",
        "classes_queued": count_classes,
        "schedules_queued": count_schedules
    }

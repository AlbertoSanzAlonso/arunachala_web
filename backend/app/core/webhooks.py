...
async def notify_n8n_content_change(
    ...
    try:
        ...
        if db_entity:
            ...
            # Include translations for multi-language RAG support
            if hasattr(db_entity, 'translations') and db_entity.translations:
                flat_payload['translations'] = db_entity.translations

            # Add publication/creation date for temporal awareness in RAG
            if hasattr(db_entity, 'created_at') and db_entity.created_at:
                # Use ISO format for standard JSON parsing
                created_dt = db_entity.created_at
                flat_payload['date'] = created_dt.isoformat() if hasattr(created_dt, 'isoformat') else str(created_dt)
            elif hasattr(db_entity, 'start_date') and db_entity.start_date:
                start_dt = db_entity.start_date
                flat_payload['date'] = start_dt.isoformat() if hasattr(start_dt, 'isoformat') else str(start_dt)
